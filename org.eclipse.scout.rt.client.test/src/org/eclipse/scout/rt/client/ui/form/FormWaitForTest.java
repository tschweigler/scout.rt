  ...
  protected void testStartAndWaitImpl(final boolean modal, final int displayHint, final String viewId) throws Exception {
    ...
    ClientSyncJob closeJob = new ClientSyncJob("Close", ClientSyncJob.getCurrentSession()) {
      ...
      protected void runVoid(IProgressMonitor monitor) throws Throwable {
        ...
        System.out.println("ClientSyncWaitForTest.testStartAndWaitImpl(...).new ClientSyncJob() {...}.runVoid() finished");
      }
    };
    closeJob.schedule();

    System.out.println(ClientSyncJob.getCurrentSession());
    System.out.println(ClientSyncJob.getCurrentSession().getDesktop());

    testSequence.add(1);
    form.start(new FormHandler());
    testSequence.add(2);

    System.out.println("ClientSyncWaitForTest.testStartAndWaitImpl() before waitFor");
    form.waitFor();

    System.out.println("ClientSyncWaitForTest.testStartAndWaitImpl() after waitFor");
    testSequence.add(5);
